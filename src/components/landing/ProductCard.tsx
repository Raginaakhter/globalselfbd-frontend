...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const discountAmount = rrp && rrp > price ? rrp - price : 0;
  const discountPercent = rrp && rrp > price ? Math.round(((rrp - price) / rrp) * 100) : 0;
  ...
  return (
    <article className="product-card group relative flex flex-col justify-between w-full overflow-hidden rounded-2xl sm:rounded-3xl border border-slate-200/80 bg-white p-3 sm:p-4 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      <div>
        {/* Top Image Section */}
        <div className={`relative w-full aspect-square rounded-xl sm:rounded-2xl overflow-hidden ${tint || "bg-slate-50"} flex items-center justify-center p-3 mb-3 transition-colors`}>
          ...
          {/* Badges Info (New / Discount / Offer badge) on Top Left */}
          <div className="absolute left-2.5 top-2.5 z-20 flex flex-col gap-1 items-start max-w-[70%]">
            {badge ? (
              <span className="inline-flex items-center rounded-lg bg-blue-600 px-2 py-0.5 text-[10px] sm:text-xs font-black uppercase tracking-wider text-white shadow-sm">
                {badge}
              </span>
            ) : discountAmount > 0 ? (
              <span className="inline-flex items-center rounded-lg bg-blue-600 px-2 py-0.5 text-[10px] sm:text-xs font-black uppercase tracking-wider text-white shadow-sm">
                {discountPercent > 0 ? `${discountPercent}% OFF` : `SAVE ${formatPrice(discountAmount)}`}
              </span>
            ) : (
              <span className="inline-flex items-center rounded-lg bg-blue-600 px-2 py-0.5 text-[10px] sm:text-xs font-black uppercase tracking-wider text-white shadow-sm">
                NEW
          ...
          {/* Product Image */}
          <Link href={`/product/${id}`} className="relative w-full h-full flex items-center justify-center overflow-hidden">
            <ProductImage
              image={image}
              emoji={emoji}
              alt={name}
              emojiClass="text-4xl sm:text-5xl drop-shadow-md transition-transform duration-500 group-hover:scale-110"
            />
          </Link>
        </div>
        ...
        <div className="flex flex-col">
          {brand && (
            <p className="text-[10px] sm:text-xs font-black uppercase tracking-wider text-slate-400 mb-1">
              {brand}
            </p>
          )}
          ...
          {/* Size / Info */}
          {size && (
            <p className="text-[11px] sm:text-xs font-medium text-slate-500 mb-2 line-clamp-1">
              {size}
            </p>
          )}
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
          <button
            ...
            className="inline-flex items-center justify-center gap-1 rounded-xl border border-blue-600/40 bg-blue-50/50 px-2 py-2 text-[11px] sm:text-xs font-bold text-blue-700 transition-all hover:bg-blue-100 hover:border-blue-600 active:scale-95 whitespace-nowrap"
          >
            {justAdded ? (
              <>
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-1 rounded-xl bg-blue-900 hover:bg-blue-950 px-2 py-2 text-[11px] sm:text-xs font-black text-white shadow-sm transition-all hover:brightness-110 active:scale-95 whitespace-nowrap"
          >
